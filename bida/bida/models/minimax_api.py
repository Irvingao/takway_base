...
class minimax_cc_api(LLMAPIBase, EmbeddingAPIBase):
    ...
    def get_chatcompletion_stream_content(self, chunk, previous_data, stream_callback, stream_callback_args) -> Tuple[Any, bool, Any]:
        ...
        if decoded_data != '' and decoded_data != None:
            print(f"decoded_data: {decoded_data}")
            # print(f"decoded_data: {decoded_data}")
            parsed_data = json.loads(decoded_data[6:])
            delta_content = parsed_data['choices'][0]['delta']
            stop = parsed_data['choices'][0].get('finish_reason', None)
            result = self.process_stream_data(stream_callback, delta_content, stream_callback_args)
        else:
            return '', None, None
    ...
    def chatcompletion(
        ...
        if chat_messages and chat_messages[0]['role'] == 'system':
            # print(f"log: 系统消息作为system_prompt: {chat_messages}")
