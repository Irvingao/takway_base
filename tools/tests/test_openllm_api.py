...
def terminal_stream_chat(api_key="xxx", 
                  base_url="http://10.10.42.227:8000/v1", 
                  model="internlm2-chat-1_8b", 
                  temperature=0.7, 
                  **kwargs):
    open_llm_api = OpenLLMAPI(api_key=api_key, base_url=base_url, model=model, stream=True)
    ...
    while True:
        ...
        for chunk in response:
            ...
            if content:
                stream_text += content
        open_llm_api.update_ai_prompt(stream_text, chat_history)
        print("AI: " + stream_text)
        
if __name__ == '__main__':
    # terminal_chat()
    terminal_stream_chat()